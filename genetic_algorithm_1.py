...
Binary genome (1 or 0), fitness score equal to summation (maximum value)
(c) AS. - 2024
"""
...
class main_ga:
    ...
    def compute_fitness(self):
        for i in range(self.n_chromosome):
            self.fitness_score.append(sum(self.population[i]))
        # print("fitness score: " + str(self.fitness_score))

    def choose_parents(self):
        fitness_sum = sum(self.fitness_score)
        for i in range(self.n_chromosome):
            self.p_chromosome.append(self.fitness_score[i] / fitness_sum)    # define the chances
